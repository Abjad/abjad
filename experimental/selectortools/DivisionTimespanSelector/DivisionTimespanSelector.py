...
class DivisionTimespanSelector(SliceTimespanSelector, TimeRelationTimespanSelector):
    r'''.. versionadded:: 1.0
    ...
    Select all divisions::
        ...
        DivisionTimespanSelector()

    Select all divisions that start during segment ``'red'``::

        >>> red_segment = selectortools.SingleSegmentTimespanSelector(identifier='red')
        >>> timespan = red_segment.timespan
        >>> time_relation = timerelationtools.timespan_2_starts_during_timespan_1(timespan_1=timespan)

    ::

        >>> division_selector = selectortools.DivisionTimespanSelector(time_relation=time_relation)
    ...
    Select the last two divisions that start during segment ``'red'``::
        ...
        >>> division_selector = selectortools.DivisionTimespanSelector(time_relation=time_relation, start_identifier=-2)
