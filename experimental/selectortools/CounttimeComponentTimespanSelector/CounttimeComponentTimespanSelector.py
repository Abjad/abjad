...
class CounttimeComponentTimespanSelector(SliceTimespanSelector, TimeRelationTimespanSelector):
    r'''.. versionadded:: 1.0
    ...
    Select the first five counttime components::
        ...
        >>> selectortools.CounttimeComponentTimespanSelector(stop_identifier=5)
        CounttimeComponentTimespanSelector(stop_identifier=5)

    Select the last five counttime components::

        >>> selectortools.CounttimeComponentTimespanSelector(start_identifier=-5)
        CounttimeComponentTimespanSelector(start_identifier=-5)

    Select counttime components from ``5`` up to but not including ``-5``::

        >>> selectortools.CounttimeComponentTimespanSelector(start_identifier=5, stop_identifier=-5)
        CounttimeComponentTimespanSelector(start_identifier=5, stop_identifier=-5)

    Select all counttime components::

        >>> selectortools.CounttimeComponentTimespanSelector()
        CounttimeComponentTimespanSelector()
    ...
    ::
        ...
        >>> measure_selector = selectortools.CounttimeComponentTimespanSelector(
        ... time_relation=time_relation, klass=Measure, start_identifier=3, stop_identifier=4)

    ::

        >>> tuplet_selector = selectortools.CounttimeComponentTimespanSelector(
        ... selector=measure_selector, klass=Tuplet, start_identifier=-1)

    ::

        >>> leaf_slice_selector = selectortools.CounttimeComponentTimespanSelector(
    ...
    def __init__(self, time_relation=None, klass=None, predicate=None, selector=None,
        ...
        assert isinstance(selector, (selectortools.SliceTimespanSelector, type(None))), repr(selector)
