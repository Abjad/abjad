...
class SingleContextDivisionSelector(BackgroundElementSelector):
    ...
    segment ``'red'``::
        ...
        >>> timespan = selectortools.SegmentSelector(index='red').timespan
        >>> inequality = timespantools.expr_starts_during_timespan(timespan=timespan)
        >>> measure = selectortools.BackgroundMeasureSelector(inequality=inequality, index=-1)
    ...
    def __init__(self, voice, inequality=None, index=0):
        ...
        BackgroundElementSelector.__init__(
            self, klass=interpretationtools.Division, index=index, inequality=inequality)
