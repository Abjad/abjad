...
class ScorePackageProxy(PackageProxy):
    ...
    def __init__(self, score_package_short_name=None, session=None):
        ...
        self._dist_proxy = scoremanagementtools.proxies.DistDirectoryProxy(
            score_package_short_name=score_package_short_name, session=self.session)
        self._etc_proxy = scoremanagementtools.proxies.EtcDirectoryProxy(
            score_package_short_name=score_package_short_name, session=self.session)
        self._exg_proxy = scoremanagementtools.proxies.ExgDirectoryProxy(
            score_package_short_name=score_package_short_name, session=self.session)
        self._mus_proxy = scoremanagementtools.proxies.MusPackageProxy(
            score_package_short_name=score_package_short_name, session=self.session)
        self._chunk_wrangler = scoremanagementtools.wranglers.ChunkPackageWrangler(
            session=self.session)
        self._material_package_wrangler = scoremanagementtools.wranglers.MaterialPackageWrangler(
            session=self.session)
        self._material_package_maker_wrangler = scoremanagementtools.wranglers.MaterialPackageMakerWrangler(
            session=self.session)
        self._music_specifier_module_wrangler = scoremanagementtools.wranglers.MusicSpecifierModuleWrangler(
            session=self.session)
    ...
    def edit_instrumentation_specifier_interactively(self):
        ...
        editor = scoremanagementtools.editors.InstrumentationEditor(session=self.session, target=target)
