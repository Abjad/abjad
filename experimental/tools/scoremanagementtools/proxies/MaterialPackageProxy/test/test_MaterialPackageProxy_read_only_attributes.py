...
def test_MaterialPackageProxy_read_only_attributes_01():
    ...
    mpp = scoremanagementtools.proxies.MaterialPackageProxy('materials.red_numbers')
    ...
    assert     mpp.material_definition_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_numbers', 'material_definition.py')
    ...
    assert     mpp.material_package_directory == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_numbers')
    ...
    assert     mpp.output_material_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_numbers', 'output_material.py')
...
def test_MaterialPackageProxy_read_only_attributes_02():
    ...
    mpp = scoremanagementtools.makers.SargassoMeasureMaterialPackageMaker('materials.red_sargasso_measures')
    ...
    assert     mpp.illustration_ly_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_sargasso_measures', 'illustration.ly')
    assert     mpp.illustration_ly_file_proxy is not None
    assert     mpp.illustration_pdf_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_sargasso_measures', 'illustration.pdf')
    ...
    assert     mpp.material_package_maker is scoremanagementtools.makers.SargassoMeasureMaterialPackageMaker
    ...
    assert     mpp.output_material_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_sargasso_measures', 'output_material.py')
    assert     mpp.output_material_module_importable_name == \
    ...
    assert      mpp.user_input_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_sargasso_measures', 'user_input.py')
...
def test_MaterialPackageProxy_read_only_attributes_03():
    ...
    mpp = scoremanagementtools.proxies.MaterialPackageProxy('materials.red_notes')
    ...
    assert     mpp.illustration_builder_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_notes', 'illustration_builder.py')
    ...
    assert     mpp.illustration_ly_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_notes', 'illustration.ly')
    assert     mpp.illustration_ly_file_proxy is not None
    assert     mpp.illustration_pdf_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_notes', 'illustration.pdf')
    assert     mpp.illustration_pdf_file_proxy is not None
    # TODO:
    #assert not mpp.is_changed
    assert not mpp.is_data_only
    ...
    assert     mpp.material_definition_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_notes', 'material_definition.py')
    assert     mpp.material_definition_module_importable_name == \
        'materials.red_notes.material_definition'
    assert     mpp.material_definition_module_proxy is not None
    assert     mpp.material_package_directory == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_notes')
    assert     mpp.material_package_maker is None
    assert     mpp.material_package_maker_class_name is None
    ...
    assert     mpp.output_material_module_file_name == \
        os.path.join(os.environ.get('SCORE_MANAGEMENT_TOOLS_MATERIALS_PATH'), 'red_notes', 'output_material.py')
    assert     mpp.output_material_module_importable_name == \
