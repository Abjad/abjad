from experimental.tools.scoremanagementtools.predicates import predicates


def test_predicates_01():

    assert predicates.is_available_underscore_delimited_lowercase_package_name('asdf')
    assert predicates.is_available_underscore_delimited_lowercase_package_name('scoremanagementtools.asdf')
    assert predicates.is_available_underscore_delimited_lowercase_package_name('materials.asdf')

    assert not predicates.is_available_underscore_delimited_lowercase_package_name('scoremanagementtools')
    assert not predicates.is_available_underscore_delimited_lowercase_package_name('materials')


def test_predicates_02():

    assert predicates.is_existing_package_name('scoremanagementtools')
    assert predicates.is_existing_package_name('materials')

    assert not predicates.is_existing_package_name('asdf')
    assert not predicates.is_existing_package_name('scoremanagementtools.asdf')
