...
#            session=session)
    ...
    def get_appropriate_material_package_proxy(self,
        ...
        if material_package_maker_class_name is None:
            material_package_proxy = scoremanagementtools.proxies.MaterialPackageProxy(
                material_package_importable_name, session=self.session)
        else:
            command = 'material_package_proxy = '
            command += 'scoremanagementtools.makers.{}(material_package_importable_name, session=self.session)'
    ...
    def make_makermade_material_package(self,
        ...
        tags = tags or {}
        command = 'from experimental.tools.scoremanagementtools.makers import {} as material_package_maker_class'.format(
