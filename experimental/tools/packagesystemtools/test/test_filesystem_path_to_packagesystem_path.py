...
def test_filesystem_path_to_packagesystem_path_01():
    ...
    assert packagesystemtools.filesystem_path_to_packagesystem_path(
        configuration.built_in_materials_directory_path, configuration) == \
        'built_in_materials'

    assert packagesystemtools.filesystem_path_to_packagesystem_path(
        configuration.built_in_specifiers_directory_path, configuration) == \
        'experimental.tools.scoremanagertools.built_in_specifiers'

    assert packagesystemtools.filesystem_path_to_packagesystem_path(
        configuration.user_sketches_directory_path, configuration) == \
        'sketches'
...
def test_filesystem_path_to_packagesystem_path_03():
    ...
    assert packagesystemtools.filesystem_path_to_packagesystem_path(
        directory_path, configuration) == 'red_example_score.foo'

    file_path = os.path.join(user_scores_directory_path, 'red_example_score', 'foo.py')
    assert packagesystemtools.filesystem_path_to_packagesystem_path(
        file_path, configuration) == 'red_example_score.foo'
