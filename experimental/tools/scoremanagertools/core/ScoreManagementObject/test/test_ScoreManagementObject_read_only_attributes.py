...
def test_ScoreManagementObject_read_only_attributes_01():
    ...
    assert score_management_object.configuration.makers_package_importable_name == 'scoremanagertools.makers'
    assert score_management_object.configuration.score_management_tools_package_importable_name == \
        'scoremanagertools'

    assert isinstance(score_management_object.session, scoremanagertools.core.Session)
