...
def test_ScoreManager_04():
    ...
    assert score_manager.session.transcript[-2][1][0] == 'Scores - active scores'

    score_manager.run(user_input='svn q')
    assert score_manager.session.transcript[-2][1][0] == 'Scores - active scores - repository commands'
