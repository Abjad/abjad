...
def test_ScoreManager_04():
    ...
    assert score_manager.session.short_transcript[-2][0] == 'Scores - active scores'

    score_manager.run(user_input='svn q')
    assert score_manager.session.short_transcript[-2][0] == 'Scores - active scores - repository commands'
...
def test_ScoreManager_07():
    ...
    assert score_manager.session.short_transcript[1] == ['> exec', '']
    assert score_manager.session.short_transcript[2] == ['XCF> 2**30']
    assert score_manager.session.short_transcript[3] == ['1073741824', '']
    assert score_manager.session.short_transcript[4] == ['> q', '']
...
def test_ScoreManager_08():
    ...
    assert score_manager.session.short_transcript[1] == ['> exec', '']
    assert score_manager.session.short_transcript[2] == ['XCF> foo']
    assert score_manager.session.short_transcript[3] == ['Expression not executable.', '']
    assert score_manager.session.short_transcript[4] == ['> q', '']
