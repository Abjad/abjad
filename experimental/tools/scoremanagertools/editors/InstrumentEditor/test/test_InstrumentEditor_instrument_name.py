...
def test_InstrumentEditor_instrument_name_01():
    ...
    score_manager._run(pending_user_input='red~example~score score~setup performers hornist horn in q')
    assert score_manager.session.io_transcript.signature == (13,)

    score_manager._run(pending_user_input='red~example~score score~setup performers hornist horn in b q')
    assert score_manager.session.io_transcript.signature == (15, (10, 13))

    score_manager._run(pending_user_input='red~example~score score~setup performers hornist horn in home q')
...
def test_InstrumentEditor_instrument_name_02():
    ...
    score_manager = scoremanagertools.scoremanager.ScoreManager()
    score_manager._run(pending_user_input='red~example~score score~setup performers hornist horn in -99 q')
