...
def test_PerformerEditor_name_01():
    ...
    score_manager._run(user_input='red~example~score setup performers hornist name q')
    assert score_manager._session.transcript.signature == (11,)

    score_manager._run(user_input='red~example~score setup performers hornist name b q')
    assert score_manager._session.transcript.signature == (13, (8, 11))

    score_manager._run(user_input='red~example~score setup performers hornist name home q')
...
def test_PerformerEditor_name_02():
    ...
    score_manager = scoremanagertools.scoremanager.ScoreManager()
    score_manager._run(user_input='red~example~score setup performers hornist name -99 q')
