...
def test_PerformerEditor_move_instrument_01():
    ...
    score_manager._run(pending_user_input="l'arch score~setup performers flutist move q")
    assert score_manager.session.io_transcript.signature == (11,)

    score_manager._run(pending_user_input="l'arch score~setup performers flutist move b q")
    assert score_manager.session.io_transcript.signature == (13, (8, 11))

    score_manager._run(pending_user_input="l'arch score~setup performers flutist move home q")
    assert score_manager.session.io_transcript.signature == (13, (0, 11))

    score_manager._run(pending_user_input="l'arch score~setup performers flutist move score q")
    assert score_manager.session.io_transcript.signature == (13, (2, 11))

    score_manager._run(pending_user_input="l'arch score~setup performers flutist move foo q")
