...
def test_PerformerEditor_add_instruments_01():
    ...
    score_manager._run(user_input='example~score~i setup perf hornist add q')
    assert score_manager._session.transcript.signature == (12,)

    score_manager._run(user_input='example~score~i setup perf hornist add b q')
    assert score_manager._session.transcript.signature == (14, (8, 12))

    score_manager._run(user_input='example~score~i setup perf hornist add home q')
    assert score_manager._session.transcript.signature == (14, (0, 12))

    score_manager._run(user_input='example~score~i setup perf hornist add score q')
    assert score_manager._session.transcript.signature == (14, (2, 12))

    score_manager._run(user_input='example~score~i setup perf hornist add foo q')
