...
class FilesystemAssetWrangler(ScoreManagerObject):
    ...
    def list_specific_asset_filesystem_paths(self, 
        ...
        if built_in_score_external:
            for directory_path in self._list_built_in_score_external_asset_container_filesystem_paths(
                head=head):
        ...
        if built_in_score_internal:
            for directory_path in self._list_built_in_score_internal_asset_container_filesystem_paths(
                head=head):
                for directory_entry in os.listdir(directory_path):
                    if directory_entry[0].isalpha():
                        filesystem_path = os.path.join(directory_path, directory_entry)
