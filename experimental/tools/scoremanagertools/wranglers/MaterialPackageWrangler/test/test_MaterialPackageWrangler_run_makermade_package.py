...
def test_MaterialPackageWrangler_run_makermade_package_01():
    ...
    assert not score_manager.configuration.packagesystem_path_exists('built_in_materials.testsargasso')

    try:
        score_manager._run(user_input='m m sargasso testsargasso default q')
        assert score_manager.configuration.packagesystem_path_exists('built_in_materials.testsargasso')
