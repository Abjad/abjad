...
def test_MaterialPackageWrangler_make_handmade_material_package_01():
    ...
    assert not wrangler.configuration.packagesystem_path_exists('built_in_materials.testnotes')


    try:
        wrangler.make_handmade_material_package('built_in_materials.testnotes')
        assert wrangler.configuration.packagesystem_path_exists('built_in_materials.testnotes')
        mpp = scoremanagertools.proxies.MaterialPackageProxy('built_in_materials.testnotes')
...
def test_MaterialPackageWrangler_make_handmade_material_package_02():
    ...
    wrangler = scoremanagertools.wranglers.MaterialPackageWrangler()
    assert wrangler.configuration.packagesystem_path_exists('built_in_materials.red_notes')
    assert py.test.raises(Exception, "wrangler.make_handmade_material_package('built_in_materials.red_notes')")
...
def test_MaterialPackageWrangler_make_handmade_material_package_04():
    ...
    try:
        ...
        wrangler.make_handmade_material_package('built_in_materials.testnotes', tags=tags)
        assert wrangler.configuration.packagesystem_path_exists('built_in_materials.testnotes')
        mpp = scoremanagertools.proxies.MaterialPackageProxy('built_in_materials.testnotes')
