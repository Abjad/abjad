...
def test_MaterialPackageWrangler_make_makermade_material_package_01():
    ...
    assert not packagepathtools.package_exists('system_materials.testsargasso')

    try:
        wrangler.make_makermade_material_package(
            'system_materials.testsargasso', 'SargassoMeasureMaterialPackageMaker')
        assert packagepathtools.package_exists('system_materials.testsargasso')
...
def test_MaterialPackageWrangler_make_makermade_material_package_02():
    ...
    assert py.test.raises(Exception,
        "wrangler.make_makermade_material_package('system_materials.red_sargasso_measures', "
...
def test_MaterialPackageWrangler_make_makermade_material_package_04():
    ...
    try:
        ...
        wrangler.make_makermade_material_package(
            'system_materials.testsargasso', 'SargassoMeasureMaterialPackageMaker', tags=tags)
        assert packagepathtools.package_exists('system_materials.testsargasso')
        mpp = scoremanagertools.materialpackagemakers.SargassoMeasureMaterialPackageMaker('system_materials.testsargasso')
        assert mpp.is_makermade
        assert mpp.directory_contents == ['__init__.py', 'tags.py', 'user_input.py']
