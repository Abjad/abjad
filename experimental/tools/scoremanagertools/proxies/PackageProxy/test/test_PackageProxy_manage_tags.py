...
def test_PackageProxy_manage_tags_01():
    ...
    score_manager._run(user_input='example~score~i tags q')
    assert score_manager._session.transcript.signature == (6,)

    score_manager._run(user_input='example~score~i tags b q')
    assert score_manager._session.transcript.signature == (8, (2, 6))

    score_manager._run(user_input='example~score~i tags home q')
    assert score_manager._session.transcript.signature == (8, (0, 6))

    score_manager._run(user_input='example~score~i tags score q')
    assert score_manager._session.transcript.signature == (8, (2, 6))

    score_manager._run(user_input='example~score~i tags foo q')
