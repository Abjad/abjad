...
def test_MaterialPackageProxy_read_only_attributes_01():
    ...
    mpp = scoremanagertools.proxies.MaterialPackageProxy('system_materials.red_numbers')
    ...
    assert     mpp.material_definition_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_numbers', 'material_definition.py')
    assert     mpp.material_definition_module_path == \
        'system_materials.red_numbers.material_definition'
    assert     mpp.material_definition_module_proxy is not None
    assert     mpp.material_package_directory == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_numbers')
    ...
    assert     mpp.output_material_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_numbers', 'output_material.py')
    assert     mpp.output_material_module_path == \
        'system_materials.red_numbers.output_material'
...
def test_MaterialPackageProxy_read_only_attributes_02():
    ...
    mpp = scoremanagertools.materialpackagemakers.SargassoMeasureMaterialPackageMaker('system_materials.red_sargasso_measures')
    ...
    assert     mpp.illustration_ly_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_sargasso_measures', 'illustration.ly')
    assert     mpp.illustration_ly_file_proxy is not None
    assert     mpp.illustration_pdf_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_sargasso_measures', 'illustration.pdf')
    ...
    assert     mpp.material_package_directory == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_sargasso_measures')
    ...
    assert     mpp.output_material_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_sargasso_measures', 'output_material.py')
    assert     mpp.output_material_module_path == \
        'system_materials.red_sargasso_measures.output_material'
    assert      mpp.output_material_module_proxy is not None
    ...
    assert      mpp.user_input_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_sargasso_measures', 'user_input.py')
    assert      mpp.user_input_module_path == \
        'system_materials.red_sargasso_measures.user_input'
...
def test_MaterialPackageProxy_read_only_attributes_03():
    ...
    mpp = scoremanagertools.proxies.MaterialPackageProxy('system_materials.red_notes')
    ...
    assert     mpp.illustration_builder_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_notes', 'illustration_builder.py')
    assert     mpp.illustration_builder_module_path == \
        'system_materials.red_notes.illustration_builder'
    assert     mpp.illustration_builder_module_proxy is not None
    assert     mpp.illustration_ly_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_notes', 'illustration.ly')
    assert     mpp.illustration_ly_file_proxy is not None
    assert     mpp.illustration_pdf_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_notes', 'illustration.pdf')
    assert     mpp.illustration_pdf_file_proxy is not None
    # TODO:
    #assert not mpp.is_changed
    assert not mpp.is_data_only
    ...
    assert     mpp.material_definition_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_notes', 'material_definition.py')
    assert     mpp.material_definition_module_path == \
        'system_materials.red_notes.material_definition'
    assert     mpp.material_definition_module_proxy is not None
    assert     mpp.material_package_directory == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_notes')
    assert     mpp.material_package_maker is None
    assert     mpp.material_package_maker_class_name is None
    ...
    assert     mpp.output_material_module_file_name == \
        os.path.join(mpp.configuration.system_materials_directory_path, 'red_notes', 'output_material.py')
    assert     mpp.output_material_module_path == \
        'system_materials.red_notes.output_material'
    assert      mpp.output_material_module_proxy is not None
    assert      mpp.should_have_illustration
