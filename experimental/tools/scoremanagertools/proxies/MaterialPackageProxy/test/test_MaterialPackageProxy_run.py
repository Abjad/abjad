...
def test_MaterialPackageProxy_run_02():
    ...
    score_manager = scoremanagertools.scoremanager.ScoreManager()
    score_manager.run(user_input='m sargasso q')
    assert score_manager.session.short_transcript[-2][0] == 'Scores - materials - sargasso multipliers'
...
def test_MaterialPackageProxy_run_03():
    ...
    assert score_manager.transcript_signature == (12, (4, 10))
...
def test_MaterialPackageProxy_run_04():
    ...
    assert score_manager.session.short_transcript[-2][0] == 'Example Score I - materials - time signatures'
