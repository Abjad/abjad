...
def test_MaterialPackageProxy_run_02():
    ...
    score_manager = scoremanagertools.scoremanager.ScoreManager()
    score_manager._run(pending_user_input='m sargasso q')
    assert score_manager.session.transcript[-2][1][0] == 'Score manager - materials - sargasso multipliers'
...
def test_MaterialPackageProxy_run_03():
    ...
    assert score_manager.session.transcript.signature == (12, (4, 10))
...
def test_MaterialPackageProxy_run_04():
    ...
    assert score_manager.session.transcript[-2][1][0] == 'Red Example Score - materials - time signatures'
