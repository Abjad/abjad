...
def test_MaterialPackageProxy_run_02():
    ...
    score_manager = scoremanagertools.scoremanager.ScoreManager()
    score_manager.run(user_input='m sargasso q')
    assert score_manager.session.transcript[-2][1][0] == 'Scores - materials - sargasso multipliers'
...
def test_MaterialPackageProxy_run_04():
    ...
    assert score_manager.session.transcript[-2][1][0] == 'Example Score I - materials - time signatures'
