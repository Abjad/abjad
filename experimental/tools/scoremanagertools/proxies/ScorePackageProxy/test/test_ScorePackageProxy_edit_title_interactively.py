...
def test_ScorePackageProxy_edit_title_interactively_01():
    ...
    try:
        ...
        assert score_manager.session.short_transcript[-5][0] == 'Betörung (2012) - setup'
        assert score_manager.session.short_transcript[-2][0] == 'Foo (2012) - setup'
    finally:
        score_manager.run(user_input='foo setup title Betörung q')
        assert score_manager.session.transcript.signature == (9,)
        assert score_manager.session.short_transcript[-5][0] == 'Foo (2012) - setup'
        assert score_manager.session.short_transcript[-2][0] == 'Betörung (2012) - setup'
