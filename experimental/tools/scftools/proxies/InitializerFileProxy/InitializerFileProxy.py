from experimental.tools.scftools.proxies.ParsableFileProxy import ParsableFileProxy
import collections
import os
...
class InitializerFileProxy(ParsableFileProxy):
    ...
    def add_safe_import_statement(self, source_module_short_name, source_attribute_name):
        safe_import_import_statement = 'from experimental.tools.scftools.helpers import safe_import\n'
    ...
    def restore_interactively(self, prompt=True):
        ...
        if 'yes'.startswith(result.lower()):
            ...
            material_package_maker_wrangler = scftools.wranglers.MaterialPackageMakerWrangler(session=self.session)
            self.push_backtrack()
