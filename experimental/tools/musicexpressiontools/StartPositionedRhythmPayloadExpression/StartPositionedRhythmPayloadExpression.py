...
class StartPositionedRhythmPayloadExpression(StartPositionedPayloadExpression):
    ...
    def __getitem__(self, expr):
        ...
        start_offset = leaves[0].get_timespan().start_offset
        stop_offset = leaves[-1].get_timespan().stop_offset
    ...
    def rotate(self, n, fracture_spanners=True):
        ...
        if isinstance(n, int):
            leaves = sequencetools.CyclicTuple(self.payload.select_leaves())
            if 0 < n:
                split_offset = leaves[-n].get_timespan().start_offset
            elif n == 0:
                return self
            else:
                split_offset = leaves[-(n+1)].get_timespan().stop_offset
            ...
            if rotation_indicator.level is None:
                ...
                for component in \
                    ...
                    score_index = component._select_parentage().score_index
            ...
            if isinstance(rotation_indicator.index, int):
                if 0 < rotation_indicator.index:
                    split_offset = components_at_level[
                        -rotation_indicator.index].get_timespan().start_offset
                elif n == 0:
                    return self
                else:
                    split_offset = components_at_level[
                        -(rotation_indicator.index+1)].get_timespan().stop_offset
            else:
        ...
        if fracture_spanners:
            ...
            for spanner in \
                ...
                spanner._components.sort(
                    lambda x, y: cmp(x._select_parentage().score_index, y._select_parentage().score_index))
