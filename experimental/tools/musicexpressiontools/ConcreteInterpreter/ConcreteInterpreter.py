...
class ConcreteInterpreter(Interpreter):
    ...
    def add_division_lists_to_score(self):
        ...
        for voice in iterationtools.iterate_voices_in_expr(self.score):
            voice_division_list = musicexpressiontools.DivisionList([], voice_name=voice.name)
    ...
    def make_payload_expressions(self, attribute):
        ...
        while region_expressions:
            ...
            for region_expression in region_expressions[:]:
                assert isinstance(region_expression, musicexpressiontools.RegionExpression)
                payload_expression = region_expression.evaluate()
                if payload_expression is not None:
                    assert isinstance(payload_expression, musicexpressiontools.StartPositionedPayloadExpression)
                    made_progress = True
    ...
    def make_rhythm_region_expressions_for_voice(self, voice_name):
        ...
        rhythm_region_division_lists = [
            musicexpressiontools.DivisionList(x, voice_name=voice_name) for x in rhythm_region_division_lists]
    ...
    def merge_prolonging_rhythm_region_expressions(self, rhythm_region_expressions):
        ...
        for rhythm_region_expression in rhythm_region_expressions:
            if result and isinstance(
                rhythm_region_expression, musicexpressiontools.SelectExpressionRhythmRegionExpression) and \
