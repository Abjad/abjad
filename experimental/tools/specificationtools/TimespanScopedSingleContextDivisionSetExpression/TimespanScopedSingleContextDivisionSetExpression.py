...
class TimespanScopedSingleContextDivisionSetExpression(TimespanScopedSingleContextSetExpression):
    ...
    def evaluate(self, voice_name):
        ...
        if isinstance(self.source_expression, specificationtools.SelectExpression):
            region_expression = specificationtools.SelectExpressionDivisionRegionExpression(
                self.source_expression, start_offset, total_duration, voice_name)
        elif isinstance(self.source_expression, specificationtools.DivisionSetExpressionLookupExpression):
            expression = self.source_expression.evaluate()
            assert isinstance(expression, specificationtools.IterablePayloadExpression)
            divisions = expression.elements
            region_expression = specificationtools.LiteralDivisionRegionExpression(
                divisions, start_offset, total_duration, voice_name)
        elif isinstance(self.source_expression, specificationtools.IterablePayloadExpression):
