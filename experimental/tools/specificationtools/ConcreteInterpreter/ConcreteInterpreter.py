...
class ConcreteInterpreter(Interpreter):
    ...
    def interpret_pitch(self):
        ...
        for pitch_set_expression in self.score_specification.pitch_set_expressions:
            assert isinstance(pitch_set_expression, specificationtools.PitchSetExpression)
            source_expression = pitch_set_expression.source_expression
            if isinstance(source_expression, specificationtools.StatalServerCursorExpression):
    ...
    def make_payload_expressions(self, attribute):
        ...
        while region_expressions:
            ...
            for region_expression in region_expressions[:]:
                assert isinstance(region_expression, specificationtools.RegionExpression)
                payload_expression = region_expression.evaluate()
                if payload_expression is not None:
                    assert isinstance(payload_expression, specificationtools.StartPositionedPayloadExpression)
                    made_progress = True
    ...
    def make_rhythm_region_expressions_for_voice(self, voice_name):
        ...
        rhythm_region_division_lists = [
            specificationtools.DivisionList(x, voice_name=voice_name) for x in rhythm_region_division_lists]
    ...
    def merge_prolonging_rhythm_region_expressions(self, rhythm_region_expressions):
        ...
        for rhythm_region_expression in rhythm_region_expressions:
            if result and isinstance(
                rhythm_region_expression, specificationtools.SelectExpressionRhythmRegionExpression) and \
