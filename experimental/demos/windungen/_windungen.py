...
def make_cyclic_matrix_for_rotation_by_bandwidth(
    staffIndexBoundsTuple, rotationBandwidth, compressedReflections = True):
...
def beam_and_fuse_beats_in_score_by_durations(score, durations, cyclic=False):
    for staff in score:
        beats = componenttools.split_components_by_durations(staff.leaves, durations, cyclic=cyclic)
...
def fuse_large_rests_of_duration_in_score_by_duration_threshold(
    ...
    for staff in score:
        bars = componenttools.split_components_by_durations(staff.leaves, [Duration(4,4)], cyclic=True, tie_split_notes=False)
...
def rotate_expression_through_adjacent_staffs_at_bandwidth_by_durations(
    ...
    splits = componenttools.split_components_by_durations(expression.leaves, durations, cyclic=cyclic, tie_split_notes=False)
