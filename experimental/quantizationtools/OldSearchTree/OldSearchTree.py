...
class OldSearchTree(datastructuretools.ImmutableDictionary):
    ...
    ::
        ...
        >>> search_tree = OldSearchTree({2: {2: None, 3: None}, 5: None})

    Return a new `OldSearchTree`.
    '''
    ...
    def offsets(self):
        ...
        objects governed by a specific `OldSearchTree` can contain.

        ::

            >>> from experimental.quantizationtools import OldSearchTree
            >>> qst = OldSearchTree({2: {3: None}})
            >>> qst.offsets
    ...
    def find_subtree_divisibility(self, parentage):
        '''Given a parentage signature, defining some subtree of a `OldSearchTree`,
        return a tuple of permitted divisions of that subtree.

        ::

            >>> from experimental.quantizationtools import OldSearchTree
            >>> qst = OldSearchTree({2: {2: None, 3: {7: None, 11: None}}, 5: None})
    ...
    def prune(self, beatspan, tempo, threshold):
        '''Prune those subtrees of a `OldSearchTree` whose divisions in milliseconds,
        ...
        ::
            ...
            >>> from experimental.quantizationtools import OldSearchTree
            >>> qst = OldSearchTree({2: {2: {2: {2: None}}}})
        ...
        if result:
            return OldSearchTree(recurse(self, beatspan))
        return result
