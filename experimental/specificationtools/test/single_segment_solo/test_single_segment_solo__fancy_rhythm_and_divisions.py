...
def test_single_segment_solo__fancy_rhythm_and_divisions_02():
    ...
    selector = red_segment.select_segment_offsets(start=(10, 16), stop=(13, 16))
    red_segment.set_divisions([(2, 32)], selector=selector)
    selector = red_segment.select_segment_ratio_part((1, 2), -1)
    red_segment.set_rhythm(library.thirty_seconds, selector=selector)
    score = score_specification.interpret()
...
def test_single_segment_solo__fancy_rhythm_and_divisions_03():
    ...
    selector = red_segment.select_segment_ratio_part((1, 1), 0)
    red_segment.set_divisions([(3, 16)], selector=selector)
    selector = red_segment.select_segment_ratio_part((1, 1), -1)
