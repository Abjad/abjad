...
class SegmentDivisionList(DivisionList):
    ...
    The best way to show this is with an example::
        ...
        Setting(Selection(contexts=['Grouped Rhythmic Staves Score'], timespan=Timespan(ScoreElementSelector(segment='1'))), 'time_signatures', [(4, 8), (3, 8)], persistent=True, truncate=False)

        >>> segment.set_divisions(segment.v1, [(3, 16)])
        Setting(Selection(contexts=['Voice 1'], timespan=Timespan(ScoreElementSelector(segment='1'))), 'divisions', [(3, 16)], persistent=True, truncate=False)

        >>> segment.set_rhythm(segment, library.thirty_seconds)
        Setting(Selection(contexts=['Grouped Rhythmic Staves Score'], timespan=Timespan(ScoreElementSelector(segment='1'))), 'rhythm', OutputBurnishedSignalFilledTimeTokenMaker('thirty_seconds'), persistent=True, truncate=False)
