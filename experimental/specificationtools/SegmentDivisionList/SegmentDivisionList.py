...
class SegmentDivisionList(DivisionList):
    ...
    The best way to show this is with an example::
        ...
        Directive(Selection(contexts=['Grouped Rhythmic Staves Score'], timespan=Timespan(start=TemporalCursor(anchor=ScoreObjectIndicator(segment='1'), edge=Left), stop=TemporalCursor(anchor=ScoreObjectIndicator(segment='1'), edge=Right))), 'time_signatures', [(4, 8), (3, 8)], persistent=True, truncate=False)

        >>> segment.set_divisions(segment.v1, [(3, 16)])
        Directive(Selection(contexts=['Voice 1'], timespan=Timespan(start=TemporalCursor(anchor=ScoreObjectIndicator(segment='1'), edge=Left), stop=TemporalCursor(anchor=ScoreObjectIndicator(segment='1'), edge=Right))), 'divisions', [(3, 16)], persistent=True, truncate=False)

        >>> segment.set_rhythm(segment, library.thirty_seconds)
        Directive(Selection(contexts=['Grouped Rhythmic Staves Score'], timespan=Timespan(start=TemporalCursor(anchor=ScoreObjectIndicator(segment='1'), edge=Left), stop=TemporalCursor(anchor=ScoreObjectIndicator(segment='1'), edge=Right))), 'rhythm', OutputBurnishedSignalFilledTimeTokenMaker('thirty_seconds'), persistent=True, truncate=False)
