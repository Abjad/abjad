...
class Selection(AbjadObject):
    ...
    ::
        ...
        Selection(timespan=Timespan(ScoreSliceIndicator(segment='red')))

    Select ``'Voice 1'`` taken over the timepsan of segment ``'red'``::

        >>> specificationtools.Selection(contexts=['Voice 1'], timespan=timespan)
        Selection(contexts=['Voice 1'], timespan=Timespan(ScoreSliceIndicator(segment='red')))
    ...
    Select ``'Voice 1'`` and ``'Voice 3'`` over the timespan of segment ``'red'``::
        ...
        Selection(contexts=['Voice 1', 'Voice 3'], timespan=Timespan(ScoreSliceIndicator(segment='red')))
