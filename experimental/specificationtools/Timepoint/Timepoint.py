...
class Timepoint(AbjadObject):
    ...
    ::
        ...
        Timepoint(anchor=ScoreSliceIndicator(segment='red'))
    ...
    Pick out the timepoint equal to the right edge of the segment with name ``'red'``::
        ...
        Timepoint(anchor=ScoreSliceIndicator(segment='red'), edge=Right)
    ...
    the segment with name ``'red'``::
        ...
        Timepoint(anchor=ScoreSliceIndicator(segment='red'), addendum=Offset(1, 8))
    ...
    Pick out the timepoint equal to one third of the way into the segment with name ``'red'``::
        ...
        Timepoint(anchor=ScoreSliceIndicator(segment='red'), edge=Right, multiplier=Fraction(1, 3))
    ...
    the segment with name ``'red'``::
        ...
        Timepoint(anchor=ScoreSliceIndicator(segment='red'), edge=Right, multiplier=Fraction(1, 3), addendum=Offset(1, 8))

    Pick out the timepoint equal to the left edge of note ``10`` in context ``'Voice 1'`` of
    the segment with name ``'red'``::

        >>> anchor = specificationtools.ScoreSliceIndicator(segment='red', context='Voice 1', klass=Note, index=10)

    ::

        >>> specificationtools.Timepoint(anchor=anchor)
        Timepoint(anchor=ScoreSliceIndicator(segment='red', context='Voice 1', klass=notetools.Note, index=10))

    Pick out the timepoint equal to the right edgright edge of note ``10`` in context ``'Voice 1'`` of
    the segment with name ``'red'``::

        >>> specificationtools.Timepoint(anchor=anchor, edge=Right)
        Timepoint(anchor=ScoreSliceIndicator(segment='red', context='Voice 1', klass=notetools.Note, index=10), edge=Right)
    ...
    def __init__(self, anchor=None, edge=None, multiplier=None, addendum=None): 
        assert isinstance(anchor, (ScoreSliceIndicator, type(None))), repr(anchor)
    ...
    def anchor(self):
        '''Timepoint anchor specified by user.
            ...
            >>> timepoint.anchor
            ScoreSliceIndicator(segment='red', context='Voice 1', klass=notetools.Note, index=10)
