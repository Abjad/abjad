def segments_to_timespan(start_segment, stop_segment=None):
    ...
    Change `start_segment` index to timespan::
        ...
        Timespan(ScoreObjectIndicator(segment=0))
    ...
    Change segments from `start_segment` name to `stop_segment` name to timespan::
        ...
        Timespan(start=Timepoint(anchor=ScoreObjectIndicator(segment='red'), edge=Left), stop=Timepoint(anchor=ScoreObjectIndicator(segment='blue'), edge=Right))
    ...
    Change segments from `start_segment` index to `stop_segment` index to timespan::
        ...
        Timespan(start=Timepoint(anchor=ScoreObjectIndicator(segment=0), edge=Left), stop=Timepoint(anchor=ScoreObjectIndicator(segment=3), edge=Right))
    ...
    start_anchor = specificationtools.ScoreObjectIndicator(segment=start_segment)
    ...
    if stop_segment is None:
        ...
        stop_anchor = specificationtools.ScoreObjectIndicator(segment=stop_segment)
