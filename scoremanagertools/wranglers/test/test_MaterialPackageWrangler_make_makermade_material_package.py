...
def test_MaterialPackageWrangler_make_makermade_material_package_01():
    ...
    assert not wrangler.configuration.packagesystem_path_exists('scoremanagertools.materialpackages.testsargasso')

    try:
        wrangler.make_makermade_material_package(
            'scoremanagertools.materialpackages.testsargasso', 'SargassoMeasureMaterialPackageMaker')
        assert wrangler.configuration.packagesystem_path_exists('scoremanagertools.materialpackages.testsargasso')
...
def test_MaterialPackageWrangler_make_makermade_material_package_02():
    ...
    assert pytest.raises(Exception,
        "wrangler.make_makermade_material_package('scoremanagertools.materialpackages.red_sargasso_measures', "
...
def test_MaterialPackageWrangler_make_makermade_material_package_04():
    ...
    try:
        ...
        wrangler.make_makermade_material_package(
            'scoremanagertools.materialpackages.testsargasso', 'SargassoMeasureMaterialPackageMaker', tags=tags)
        assert wrangler.configuration.packagesystem_path_exists('scoremanagertools.materialpackages.testsargasso')
        mpp = scoremanagertools.materialpackagemakers.SargassoMeasureMaterialPackageMaker('scoremanagertools.materialpackages.testsargasso')
        assert mpp.is_makermade
        assert mpp._list_directory() == [
