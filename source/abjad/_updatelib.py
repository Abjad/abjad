...
def _get_after_grace_leaf_offsets(
    ...
    main_leaf_stop_offset = main_leaf._timespan.stop_offset
    ...
    if displacement == _duration.Duration(0):
...
def _get_before_grace_leaf_offsets(
    ...
    container = leaf._parent
    main_leaf = container._main_leaf
    main_leaf_start_offset = main_leaf._timespan.start_offset
...
def _get_obgc_leaf_offsets(leaf) -> tuple[_duration.Offset, _duration.Offset]:
    ...
    first_nongrace_leaf_start_offset = first_nongrace_leaf._timespan.start_offset
    ...
    if start_displacement == _duration.Duration(0):
    ...
    for component in _iterate_entire_score(root):
        ...
        if len(indicators) == 1:
            ...
            if not metronome_mark.is_imprecise():
                ...
                pairs.append(pair)
        all_stop_offsets.add(component._timespan.stop_offset)
...
def _to_measure_number(
    ...
    if displacement is not None:
        ...
        if (
            displacement < _duration.Duration(0)
...
def _update_component_offsets(component) -> None:
    if isinstance(component, _score.BeforeGraceContainer):
        ...
        stop_offset_duration = (
            _duration.Duration(*start_offset.fraction.as_integer_ratio())
