...
from scoremanager.ide.AssetController import AssetController
...
class AbjadIDE(AssetController):
    ...
    def _build_file_wrangler(self):
        from scoremanager import ide
        return ide.BuildFileWrangler(session=self._session)

    @property
    @systemtools.Memoize
    def _distribution_file_wrangler(self):
        from scoremanager import ide
        return ide.DistributionFileWrangler(session=self._session)
    ...
    def _maker_file_wrangler(self):
        from scoremanager import ide
        return ide.MakerFileWrangler(session=self._session)

    @property
    @systemtools.Memoize
    def _material_package_wrangler(self):
        from scoremanager import ide
        return ide.MaterialPackageWrangler(session=self._session)

    @property
    @systemtools.Memoize
    def _score_package_wrangler(self):
        from scoremanager import ide
        return ide.ScorePackageWrangler(session=self._session)

    @property
    @systemtools.Memoize
    def _segment_package_wrangler(self):
        from scoremanager import ide
        return ide.SegmentPackageWrangler(session=self._session)

    @property
    @systemtools.Memoize
    def _stylesheet_wrangler(self):
        from scoremanager import ide
        return ide.StylesheetWrangler(session=self._session)
    ...
    def _run(self, input_=None):
        ...
        controller = ide.ControllerContext(
            controller=self,
