...
class Session(abctools.AbjadObject):
    ...
    ..  container:: example
        ...
        ::
            ...
            >>> session_in_score = scoremanager.ide.Session()
    ...
    def __init__(self, input_=None, is_test=False):
        ...
        self._configuration = ide.Configuration()
        self._confirm = True
        ...
        self._io_manager = ide.IOManager(session=self)
    ...
    def _format_controller_breadcrumbs(self):
        ...
        for controller in self.controller_stack[1:]:
            if isinstance(controller, ide.Selector):
                continue
    ...
    def _set_test_score(self, score_package_name):
        ...
        manager = ide.ScorePackageManager(
            path=path,
            session=self,
            )
    ...
    def controller_stack(self):
        ...
        Returns list of objects all of which are either wranglers or ide.
        '''
    ...
    def current_score_package_manager(self):
        ...
        for controller in reversed(self.controller_stack):
            if isinstance(controller, ide.ScorePackageManager):
                return controller
    ...
    def is_autoadding(self):
        ...
        from scoremanager import ide
        for controller in reversed(self.controller_stack):
            if isinstance(controller, ide.Autoeditor):
    ...
    def is_in_user_input_getter(self):
        ...
        from scoremanager import ide
        for controller in reversed(self.controller_stack):
            if isinstance(controller, ide.UserInputGetter):
                return True
        return False
