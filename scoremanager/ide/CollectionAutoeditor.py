...
from scoremanager.ide.Autoeditor import Autoeditor
...
class CollectionAutoeditor(Autoeditor):
    ...
    def __init__(
        ...
        self._item_getter_configuration_method = \
            ide.UserInputGetter.append_expr
        ...
        if hasattr(target, '_item_creator_class'):
            ...
            if isinstance(dummy_item, datastructuretools.TypedList):
                self._item_creator_class = ide.ListAutoeditor
            else:
                self._item_creator_class = ide.Autoeditor
    ...
    def edit_item(self, number):
        ...
        if item is None:
            return
        item_editor_class = self._item_editor_class or ide.Autoeditor
