...
def test_PerformerEditor_move_instrument_01():
    ...
    string = 'green~example~score score~setup instrumentation flutist move q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (11,)

    string = 'green~example~score score~setup instrumentation flutist move b q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (13, (8, 11))

    string = 'green~example~score score~setup instrumentation flutist move h q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (13, (0, 11))

    string = 'green~example~score score~setup instrumentation flutist move s q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (13, (2, 11))

    string = 'green~example~score score~setup instrumentation'
