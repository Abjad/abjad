...
def test_InstrumentationEditor_run_01():
    ...
    string = 'red~example~score score~setup instrumentation q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (8,)

    string = 'red~example~score score~setup instrumentation b q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (10, (4, 8))

    string = 'red~example~score score~setup instrumentation home q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (10, (0, 8))

    string = 'red~example~score score~setup instrumentation score q'
    score_manager._run(pending_user_input=string)
    assert score_manager.session.io_transcript.signature == (10, (2, 8))

    string = 'red~example~score score~setup instrumentation foo q'
    score_manager._run(pending_user_input=string)
