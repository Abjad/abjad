...
def test_ScorePackageManager_interactively_edit_forces_tagline_01():
    ...
    assert score_manager._session.io_transcript.signature == (7,)

    string = 'red~example~score setup tagline b q'
    score_manager._run(pending_user_input=string)
    assert score_manager._session.io_transcript.signature == (9, (4, 7))

    string = 'red~example~score setup tagline s q'
    score_manager._run(pending_user_input=string)
    assert score_manager._session.io_transcript.signature == (9, (2, 7))

    string = 'red~example~score setup tagline h q'
    score_manager._run(pending_user_input=string)
    assert score_manager._session.io_transcript.signature == (9, (0, 7))
