...
def test_MaterialManager_add_metadatum_01():
    ...
    input_ = 'm example~numbers mdg flavor_type default q'
    score_manager._run(pending_user_input=input_)
    assert score_manager._transcript.entries[-4].title == 'None'

    # add flavor_type metadatum
    input_ = 'm example~numbers mda flavor_type cherry q'
    score_manager._run(pending_user_input=input_)
    ...
    input_ = 'm example~numbers mdrm flavor_type default q'
    score_manager._run(pending_user_input=input_)
