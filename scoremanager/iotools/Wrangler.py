...
class Wrangler(AssetController):
    ...
    def __init__(self, session=None):
        ...
        self._manager_class = iotools.PackageManager
    ...
    def _find_up_to_date_manager(
        ...
        if type(self) is iotools.ScorePackageWrangler:
            if system:
                scores_directory = \
                    self._configuration.example_score_packages_directory
            else:
    ...
    def _make_storehouse_menu_entries(
        ...
        wrangler = iotools.ScorePackageWrangler(session=self._session)
    ...
    def _remove_assets(self):
        ...
        if self._session.confirm:
            ...
            if not result == confirmation_string:
                return
        for path in paths:
            manager = iotools.PackageManager(path=path, session=self._session)
            with self._io_manager._make_silent():
                manager._remove()
        self._session._is_pending_output_removal = True
