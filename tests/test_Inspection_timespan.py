...
def test_Inspection_timespan_01():
    ...
    for i, x in enumerate(voice):
        assert abjad.inspect(x).timespan().start_offset == i * abjad.Offset(
            1, 8
        )
...
def test_Inspection_timespan_08():
    ...
    voice = abjad.Voice(
        [abjad.Note(0, (1, 8)), tuplet_1, abjad.Note(0, (1, 8))]
    )
    offset = 0
...
def test_Inspection_timespan_09():
    ...
    tuplet_1 = abjad.Tuplet((2, 3), "c'8 c'8 c'8")
    tuplet = abjad.Tuplet(
        (2, 3), [abjad.Note("c'4"), tuplet_1, abjad.Note("c'4")]
    )
    offset = 0
...
def test_Inspection_timespan_19():
    ...
    voice = abjad.Voice(
        3 * abjad.Tuplet(abjad.Multiplier(2, 3), "c'8 d'8 e'8")
    )
    assert abjad.inspect(voice[0]).timespan().start_offset == 0 * abjad.Offset(
        1, 4
    )
    assert abjad.inspect(voice[1]).timespan().start_offset == 1 * abjad.Offset(
        1, 4
    )
    assert abjad.inspect(voice[2]).timespan().start_offset == 2 * abjad.Offset(
        1, 4
    )
...
def test_Inspection_timespan_20():
    ...
    assert abjad.inspect(voice[2]).timespan().start_offset == 3 * abjad.Offset(
        1, 8
    )
...
def test_Inspection_timespan_21():
    ...
    assert abjad.inspect(
        tuplet[0]
    ).timespan().start_offset == 0 * abjad.Offset(1, 6)
    assert abjad.inspect(
        tuplet[1]
    ).timespan().start_offset == 1 * abjad.Offset(1, 6)
    assert abjad.inspect(
        tuplet[2]
    ).timespan().start_offset == 2 * abjad.Offset(1, 6)
...
def test_Inspection_timespan_22():
    ...
    assert abjad.inspect(inner_voice).timespan().start_offset == abjad.Offset(
        2, 8
    )
    assert abjad.inspect(outer_voice).timespan().start_offset == abjad.Offset(
        1, 8
    )
...
def test_Inspection_timespan_24():
    ...
    assert abjad.inspect(voice_1b).timespan().start_offset == abjad.Offset(
        4, 8
    )
    assert abjad.inspect(voice_2b).timespan().start_offset == abjad.Offset(
        4, 8
    )
