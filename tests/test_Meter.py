...
def test_Meter___init___01():
    ...
    assert abjad.Meter(rtc, do_not_populate=True).rtm_format == "(2/4 (1/4 1/4))"


def test_Meter___init___02():
    rtc = abjad.meter.make_best_guess_rtc((3, 4))
    assert abjad.Meter(rtc, do_not_populate=True).rtm_format == "(3/4 (1/4 1/4 1/4))"


def test_Meter___init___03():
    rtc = abjad.meter.make_best_guess_rtc((4, 4))
    assert (
        abjad.Meter(rtc, do_not_populate=True).rtm_format == "(4/4 (1/4 1/4 1/4 1/4))"
    )
...
def test_Meter___init___04():
    ...
    assert abjad.Meter(rtc, do_not_populate=True).rtm_format == string
...
def test_Meter___init___06():
    ...
    abjad.Meter(rtc, do_not_populate=True).rtm_format == string


def test_Meter___init___07():
    rtc = abjad.meter.make_best_guess_rtc((1, 4))
    assert abjad.Meter(rtc, do_not_populate=True).rtm_format == "(1/4 (1/4))"
