...
class _Leaf(_Component):
   ...
   def _operate(self, arg, operator):
      ...
      self_pairs = set([(x.pitch_class_name, x.octave_number) 
         for x in pitchtools.list_named_pitches_in_expr(self) if x is not None])
      arg_pairs = set([(x.pitch_class_name, x.octave_number) 
         for x in pitchtools.list_named_pitches_in_expr(arg) if x is not None])
