...
def test_treetools_explode_overlapping_tree_into_nonoverlapping_trees_uncompactly_01( ):
    ...
    xtrees = explode_overlapping_tree_into_nonoverlapping_trees_uncompactly(tree)
    assert len(xtrees) == max([interval.data['depth'] for interval in dtree])
    
def test_treetools_explode_overlapping_tree_into_nonoverlapping_trees_uncompactly_02( ):
    '''All resulting trees are non-zero in length.'''
    tree = IntervalTree(_make_test_blocks( ))
    xtrees = explode_overlapping_tree_into_nonoverlapping_trees_uncompactly(tree)
    assert all([len(xtree) for xtree in xtrees])
    
def test_treetools_explode_overlapping_tree_into_nonoverlapping_trees_uncompactly_03( ):
