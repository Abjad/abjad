...
def test_IntervalTree_shift_member_interval_by_value_01( ):
    ...
    assert len(old) == 1
    tree.shift_member_interval_by_value(old[0], -108)
    new = tree.find_intervals_starting_at_offset(-100)
    assert len(new) == 1
    assert old != new
    assert old[0] not in tree

def test_IntervalTree_shift_member_interval_by_value_02( ):
    '''Non-member intervals cannot be altered.'''
    tree = IntervalTree(_make_test_blocks( ))
    nonmember = Block(-3, 2, 'non-member')
    py.test.raises(AssertionError,
        'tree.shift_member_interval_by_value(nonmember, 3)')
