def ordered_pitch_class_numbers_are_within_ordered_pitch_numbers(pcs, pitches):
   '''True if `pcs` all appear
   in octave-relative order in `pitches`. ::

      abjad> pcs = [2, 7, 10]
      abjad> pitches = [6, 9, 12, 13, 14, 19, 22, 27, 28, 29, 32, 35]
      abjad> pitchtools.ordered_pitch_class_numbers_are_within_ordered_pitch_numbers(pcs, pitches)
      True

   .. todo:: extend ``pitchtools.ordered_pitch_class_numbers_are_within_ordered_pitch_numbers( )`` to work on 
