...
def list_named_chromatic_pitch_carriers_in_expr_sorted_by_numeric_chromatic_pitch_class(
   pitch_carriers):
   '''.. versionadded:: 1.1.2
   
   List named chromatic pitch carriers in `expr` sorted by numeric chromatic pitch class::

      abjad> chord = Chord([9, 11, 12, 14, 16], (1, 4))
      abjad> notes = chordtools.arpeggiate_chord(chord)
      abjad> pitchtools.list_named_chromatic_pitch_carriers_in_expr_sorted_by_numeric_chromatic_pitch_class(notes)      
