...
def transpose_pitch_by_melodic_chromatic_interval(pitch_carrier, melodic_chromatic_interval):
   '''.. versionadded:: 1.1.2
   ...
   Transpose `pitch_carrier` by `melodic_chromatic_interval`. ::
      ...
      abjad> pitchtools.transpose_pitch_by_melodic_chromatic_interval(pitch, mci)
      NamedPitch(a, 4)

   .. versionchanged:: 1.1.2
      renamed ``pitchtools.transpose_by_melodic_chromatic_interval( )`` to
      ``pitchtools.transpose_pitch_by_melodic_chromatic_interval( )``.
   '''
   ...
   if isinstance(pitch_carrier, NamedPitch):
      return NamedPitch(pitch_carrier.pitch_number + melodic_chromatic_interval.number)
      ...
      new_pitch = NamedPitch(
         pitch_carrier.pitch.pitch_number + melodic_chromatic_interval.number)
      ...
      for new_nh, old_nh in zip(
         ...
         new_pitch = NamedPitch(
            old_nh.pitch.pitch_number + melodic_chromatic_interval.number)
