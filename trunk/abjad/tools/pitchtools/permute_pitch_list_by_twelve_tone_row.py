...
def permute_pitch_list_by_twelve_tone_row(pitches, row):
   '''Permute `pitches` by `row`. 
   
   ::

      abjad> notes = notetools.make_notes([17, -10, -2, 11], [Rational(1, 4)])
      abjad> row = pitchtools.TwelveToneRow([10, 0, 2, 6, 8, 7, 5, 3, 1, 9, 4, 11])
      abjad> pitchtools.permute_pitch_list_by_twelve_tone_row(notes, row)
   ...
   .. versionchanged:: 1.1.2
      ...
      ``pitchtools.permute_pitch_list_by_twelve_tone_row( )``.
   '''
