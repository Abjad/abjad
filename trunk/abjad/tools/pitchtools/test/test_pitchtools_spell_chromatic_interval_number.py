...
def test_pitchtools_spell_chromatic_interval_number_01():
    ...
    assert diatonic_interval == pitchtools.NamedMelodicInterval('perfect', 1)

    diatonic_interval = pitchtools.spell_chromatic_interval_number(1, 1)
    assert diatonic_interval == pitchtools.NamedMelodicInterval(
        'augmented', 1)

    diatonic_interval = pitchtools.spell_chromatic_interval_number(2, 0)
    assert diatonic_interval == pitchtools.NamedMelodicInterval('diminished', 2)

    diatonic_interval = pitchtools.spell_chromatic_interval_number(2, 1)
    assert diatonic_interval == pitchtools.NamedMelodicInterval('minor', 2)

    diatonic_interval = pitchtools.spell_chromatic_interval_number(2, 2)
    assert diatonic_interval == pitchtools.NamedMelodicInterval('major', 2)

    diatonic_interval = pitchtools.spell_chromatic_interval_number(2, 3)
    assert diatonic_interval == pitchtools.NamedMelodicInterval('augmented', 2)
