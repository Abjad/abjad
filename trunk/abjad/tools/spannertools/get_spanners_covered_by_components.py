...
def get_spanners_covered_by_components(components):
    ...
    ``t.start_ofset <= p.get_timespan().start_offset and 
    p.get_timespan().stop_offset <= t.stop_offset``.
    '''
    ...
    components_begin = first.get_timespan().start_offset
    components_end = last.get_timespan().stop_offset
