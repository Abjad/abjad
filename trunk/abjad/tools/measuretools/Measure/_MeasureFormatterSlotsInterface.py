...
class _MeasureFormatterSlotsInterface(_ContainerFormatterSlotsInterface):
   ...
   def slot_3(self):
      ...
      result.append([('comment_marks', ''), _get_comment_format_contributions_for_slot(measure, 'opening')])
      result.append([('overrides', 'overrides'), _get_grob_override_format_contributions(self._client._client)])
      result.append([('settings', 'settings'), _get_context_setting_format_contributions(measure)])
      result.append([('context_marks', 'context_marks'), _get_context_mark_format_contributions_for_slot(measure, 'opening')])
