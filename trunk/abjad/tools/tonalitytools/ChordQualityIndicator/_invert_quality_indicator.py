def _invert_quality_indicator(intervals, inversion):
   ...
   if isinstance(inversion, int):
      ...
      intervals = listtools.rotate(intervals, -inversion)
      rotation = -inversion
   elif inversion == 'root':
      rotation = 0
   elif inversion == 'first':
      #self.rotate(-1)
      intervals = listtools.rotate(intervals, -1)
      rotation = -1
   elif inversion == 'second':
      #self.rotate(-2)
      intervals = listtools.rotate(intervals, -2)
      rotation = -2
   elif inversion == 'third':
      #self.rotate(-3)
      intervals = listtools.rotate(intervals, -3)
      rotation = -3
   elif inversion == 'fourth':
      #self.rotate(-4)
      intervals = listtools.rotate(intervals, -4)
      rotation = -4
   else:
