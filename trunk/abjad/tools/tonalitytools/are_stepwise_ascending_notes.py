...
def are_stepwise_ascending_notes(*expr):
   ...
   True when notes in `expr` are stepwise ascneding. ::
      ...
      abjad> tonalitytools.are_stepwise_notes_ascending(t[:])
      True

   Otherwise false. ::

      abjad> tonalitytools.are_stepwise_notes_ascending(Note(0, (1, 4)), Note(0, (1, 4)))
      False
   ...
   for left, right in listtools.iterate_sequence_pairwise(componenttools.iterate_components_forward_in_expr(expr, Note)):
      try:
