...
def thread_backward_from_component(component, klass = None):
   ...
   Starting from the last leaf in score. ::
      ...
      abjad> for x in iterate.thread_backward_from_component(staff.leaves[-1], Note):
      ...     x
      ... 
      Note(c', 8)
      Note(d', 8)
   ...
   Yield all components in thread. ::
      ...
      abjad> for x in iterate.thread_backward_from_component(staff.leaves[-1]):
      ...     x
      ... 
      Note(c'', 8)
      Voice{2}
   ...
   Compare with :func:`iterate.thread_backward_in_expr() 
   <abjad.tools.iterate.thread_backward_in_expr>`.

   .. versionchanged:: 1.1.2
      renamed ``iterate.thread_backward_from( )`` to
      ``iterate.thread_backward_from_component( )``.
   '''
