...
def thread_forward_from_component(component, klass = None):
   ...
   Starting from the first leaf in score. ::
      ...
      abjad> for x in iterate.thread_forward_from_component(staff.leaves[0], Note):
      ...     x
      ... 
      Note(c', 8)
      Note(d', 8)
   ...
   Starting from the second leaf in score. ::
      ...
      abjad> for x in iterate.thread_forward_from_component(staff.leaves[1], Note):
      ...     x
      ... 
      Note(d', 8)
      Note(g', 8)
   ...
   Yield all components in thread. ::
      ...
      abjad> for x in iterate.thread_forward_from_component(staff.leaves[0]):
      ...     x
      ... 
      Note(c', 8)
      Voice{2}
   ...
   Compare with :func:`iterate.thread_forward_in_expr() 
   <abjad.tools.iterate.thread_forward_in_expr>`.

   .. versionchanged:: 1.1.2
      renamed ``iterate.thread_forward_from( )`` to
      ``iterate.thread_forward_from_component( )``.
   '''
