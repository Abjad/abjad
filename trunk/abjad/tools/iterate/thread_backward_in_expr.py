def thread_backward_in_expr(expr, klass, thread_signature):
   ...
   ::
      ...
      abjad> for x in iterate.thread_backward_in_expr(staff, Note, signature):
      ...     x
      ... 
      Note(c'', 8)
      Note(b', 8)
   ...
   Compare with :func:`iterate.naive_backward_in_expr() 
   <abjad.tools.iterate.naive_backward>`.

   .. versionchanged:: 1.1.2
      renamed ``iterate.thread_backward_in( )`` to
      ``iterate.thread_backward_in_expr( )``.
   '''
   ...
   if isinstance(expr, (list, tuple)):
      for m in reversed(expr):
         for x in thread_backward_in_expr(m, klass, thread_signature):
            yield x
