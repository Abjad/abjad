...
from abjad.tools.iterate.timeline_backward_in_expr import timeline_backward_in_expr


def timeline_backward_from_component(expr, klass = _Leaf):
   ...
   starting from `expr`. ::
      ...
      >>
      abjad> for leaf in iterate.timeline_backward_from_component(score[1][2]):
      ...     leaf
      ... 
      Note(b', 8)
      Note(c', 4)
   ...
   .. versionchanged:: 1.1.2
      ...
      ``iterate.timeline_backward_from_component( )``.
   '''

   root = expr.parentage.root
   component_generator = timeline_backward_in_expr(root, klass = klass)
