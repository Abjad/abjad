...
def naive_forward_in_expr(expr, klass = _Leaf, start = 0, stop = None):
   r'''Yield left-to-right instances of `klass` in `expr`. ::
      ...
      abjad> for x in iterate.naive_forward_in_expr(staff, Note):
      ...     x
      ... 
      Note(c', 8)
      Note(d', 8)
   ...
   ::
      ...
      abjad> for x in iterate.naive_forward_in_expr(staff, Note, start = 0, stop = 4):
      ...     x
      ... 
      Note(c', 8)
      Note(d', 8)
   ...
   ::
      ...
      abjad> for x in iterate.naive_forward_in_expr(staff, Note, start = 4):
      ...     x
      ... 
      Note(g', 8)
      Note(a', 8)
   ...
   ::
      ...
      abjad> for x in iterate.naive_forward_in_expr(staff, Note, start = 4, stop = 6):
      ...     x
      ... 
      Note(g', 8)
      Note(a', 8)   
   ...
   .. versionchanged:: 1.1.2
      Renamed from ``iterate.naive( )`` to ``iterate.naive_forward_in_expr( )``.

   .. versionchanged:: 1.1.2
      `klass` now defaults to ``_Leaf``.

   .. versionchanged:: 1.1.2
      renamed ``iterate.naive_forward_in( )`` to
      ``iterate.naive_forward_in_expr( )``.
   '''
