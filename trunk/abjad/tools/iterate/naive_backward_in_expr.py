...
def naive_backward_in_expr(expr, klass = _Leaf, start = 0, stop = None):
   ...
   ::
      ...
      abjad> for x in iterate.naive_backward_in_expr(staff, Note):
      ...     x 
      ... 
      Note(a', 8)
      Note(g', 8)
   ...
   ::
      ...
      abjad> for x in iterate.naive_backward_in_expr(staff, Note, start = 0, stop = 4):
      ...     x
      ... 
      Note(a', 8)
      Note(g', 8)
   ...
   ::
      ...
      abjad> for x in iterate.naive_backward_in_expr(staff, Note, start = 4):
      ...     x
      ... 
      Note(d', 8)
      Note(c', 8)
   ...
   ::
      ...
      abjad> for x in iterate.naive_backward_in_expr(staff, Note, start = 4, stop = 6):
      ...     x
      ... 
      Note(d', 8)
      Note(c', 8)
   ...
   .. versionchanged:: 1.1.2
      Renamed from ``iterate.backwards( )`` to ``iterate.naive_backward_in_expr( )``.

   .. versionchanged:: 1.1.2
      renamed ``iterate.naive_backward_in( )`` to
      ``iterate.naive_backward_in_expr( )``.
   '''
