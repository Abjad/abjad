def thread_forward_in_expr(expr, klass, thread_signature):
   ...
   ::
      ...
      abjad> for x in iterate.thread_forward_in_expr(staff, Note, signature):
      ...     x
      ... 
      Note(c', 8)
      Note(d', 8)
   ...
   Compare with :func:`iterate.naive_forward_in_expr() 
   <abjad.tools.iterate.naive_forward>`.

   .. versionchanged:: 1.1.2
      renamed ``iterate.thread_forward_in( )`` to
      ``iterate.thread_forward_in_expr( )``.
   '''
   ...
   if isinstance(expr, (list, tuple)):
      for m in expr:
         for x in thread_forward_in_expr(m, klass, thread_signature):
            yield x
