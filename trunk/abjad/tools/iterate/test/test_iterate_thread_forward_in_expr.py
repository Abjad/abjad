...
def test_iterate_thread_forward_in_expr_01( ):
   '''Yield nothing when class not present.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   iter = iterate.thread_forward_in_expr(t, Rest, t[0].thread.signature)
...
def test_iterate_thread_forward_in_expr_03( ):
   '''Yield exact leaves.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   iter = iterate.thread_forward_in_expr(t, Note, t[0].thread.signature)
...
def test_iterate_thread_forward_in_expr_05( ):
...
def test_iterate_thread_forward_in_expr_06( ):
...
def test_iterate_thread_forward_in_expr_07( ):
   '''Yield only Notes matching the given thread signature.'''
   v1 = Voice(Note(0, (1, 4)) * 2)
   v2 = Voice(Note(2, (1, 4)) * 2)
   v1.name = 'flute'
