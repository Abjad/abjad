...
def test_iterate_measures_backward_in_01( ):
   ...
   measures = list(iterate.measures_backward_in(staff))
...
def test_iterate_measures_backward_in_02( ):
   '''Optional start and stop keyword paramters.'''

   staff = Staff(RigidMeasure((2, 8), construct.run(2)) * 3)
   pitchtools.diatonicize(staff)

   measures = list(iterate.measures_backward_in(staff, start = 1))
   assert measures[0] is staff[1]
   assert measures[1] is staff[0]
   assert len(measures) == 2

   measures = list(iterate.measures_backward_in(staff, stop = 2))
   assert measures[0] is staff[2]
   assert measures[1] is staff[1]
   assert len(measures) == 2
