...
def test_iterate_naive_forward_in_expr_01( ):
   '''Yield nothing when class not present.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   iter = iterate.naive_forward_in_expr(t, Rest)
   assert len(list(iter)) == 0


def test_iterate_naive_forward_in_expr_02( ):
   '''Yield topmost node only.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   iter = iterate.naive_forward_in_expr(t, Staff)
...
def test_iterate_naive_forward_in_expr_04( ):
   '''Yield exact leaves.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   iter = iterate.naive_forward_in_expr(t, Note)
   assert len(list(iter)) == 9


def test_iterate_naive_forward_in_expr_05( ):
   '''Yield leaves based on names higher in inheritence hierarchy.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   #from abjad.leaf import _Leaf
   iter = iterate.naive_forward_in_expr(t, _Leaf)
   assert len(list(iter)) == 9
...
def test_iterate_naive_forward_in_expr_07( ):
   '''Yield all python objects.'''
   t = Staff(FixedDurationTuplet((2, 4), Note(0, (1, 4)) * 3) * 3)
   iter = iterate.naive_forward_in_expr(t, object)
   assert len(list(iter)) == 13


def test_iterate_naive_forward_in_expr_08( ):
   '''From start index.'''
   ...
   leaves = list(iterate.naive_forward_in_expr(t, _Leaf, 3))
...
def test_iterate_naive_forward_in_expr_09( ):
   '''To stop index.'''
   ...
   leaves = list(iterate.naive_forward_in_expr(t, _Leaf, 0, 3))
...
def test_iterate_naive_forward_in_expr_10( ):
   ...
   leaves = list(iterate.naive_forward_in_expr(t, _Leaf, 1, 5))
