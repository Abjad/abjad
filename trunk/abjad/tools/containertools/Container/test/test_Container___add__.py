...
def test_Container___add___03():
    ...
    assert inspect(tadd).is_well_formed()
...
def test_Container___add___07():
    ...
    tadd = t1 + t2
    assert inspect(tadd).is_well_formed()
    assert inspect(t1).is_well_formed()
    assert inspect(t2).is_well_formed()
...
def test_Container___add___09():
    ...
    v1 += v2
    assert inspect(v1).is_well_formed()
    assert inspect(v2).is_well_formed()
    assert len(v1) == 8
    assert len(v2) == 4
