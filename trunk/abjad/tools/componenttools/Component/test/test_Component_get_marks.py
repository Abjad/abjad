...
def test_Component_get_marks_01():
...
def test_Component_get_marks_02():
...
def test_Component_get_marks_03():
    ...
    context_marks = staff[0].get_marks(contexttools.ContextMark)

    assert dynamic_mark in context_marks
    assert len(context_marks) == 1


def test_Component_get_marks_04():
    ...
    dynamic_marks = staff[0].get_marks(contexttools.DynamicMark)

    assert dynamic_mark in dynamic_marks
    assert len(dynamic_marks) == 1


def test_Component_get_marks_05():
...
def test_Component_get_marks_06():
    ...
    marks = staff[0].get_marks(marktools.LilyPondComment)
...
def test_Component_get_marks_07():

    note = Note("c'4")
    stem_tremolo = marktools.StemTremolo(16)(note)
    stem_tremolos = note.get_marks(marktools.StemTremolo)
