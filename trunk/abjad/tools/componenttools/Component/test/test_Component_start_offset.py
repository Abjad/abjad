...
def test_Component_start_offset_07():
    ...
    for i, x in enumerate(t):
        assert x.start_offset == i * Duration(1, 12)
...
def test_Component_start_offset_11():
    ...
    for i, x in enumerate(v.leaves):
        assert x.start_offset == i * Duration(1, 8) + Duration(1, 8)
...
def test_Component_start_offset_12():
    ...
    for i, x in enumerate(v2.leaves):
        assert x.start_offset == i * Duration(1, 8) + Duration(1, 2)
...
def test_Component_start_offset_16():
    ...
    for i, x in enumerate(t):
        assert x.start_offset == i * Duration(4, 8)
...
def test_Component_start_offset_17():
    ...
    assert t[0].start_offset == Duration(0)
    assert t[1].start_offset == Duration(1, 2)
...
def test_Component_start_offset_19():
    ...
    assert t[0].start_offset == 0 * Duration(1, 4)
    assert t[1].start_offset == 1 * Duration(1, 4)
    assert t[2].start_offset == 2 * Duration(1, 4)
...
def test_Component_start_offset_20():
    ...
    t = Voice([Note(0, (1, 8)), tp, Note(0, (1, 8))])
    assert t[0].start_offset == 0 * Duration(1, 8)
    assert t[1].start_offset == 1 * Duration(1, 8)
    assert t[2].start_offset == 3 * Duration(1, 8)
...
def test_Component_start_offset_21():
    ...
    tp = tuplettools.FixedDurationTuplet(Duration(1, 4), notetools.make_repeated_notes(3))
    t = tuplettools.FixedDurationTuplet(Duration(2, 4), [Note("c'4"), tp, Note("c'4")])
    assert t[0].start_offset == 0 * Duration(1, 6)
    assert t[1].start_offset == 1 * Duration(1, 6)
    assert t[2].start_offset == 2 * Duration(1, 6)
...
def test_Component_start_offset_22():
    ...
    assert vin.start_offset == Duration(2, 8)
    assert vout.start_offset == Duration(1, 8)
...
def test_Component_start_offset_23():
    ...
    t = Staff([v1, v2])
    t.is_parallel = True
    assert t[0].start_offset == 0
    assert t[1].start_offset == 0
...
def test_Component_start_offset_24():
    ...
    assert v1.start_offset == 0
    assert v2.start_offset == 0
    assert v1b.start_offset == Duration(4, 8)
    assert v2b.start_offset == Duration(4, 8)
