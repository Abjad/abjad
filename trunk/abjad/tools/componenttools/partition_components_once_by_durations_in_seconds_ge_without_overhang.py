...
def partition_components_once_by_durations_in_seconds_ge_without_overhang(components, durations_in_seconds):
   '''Partition `components` once by durations in seconds that
   equal or are just greater than `durations_in_seconds` and
