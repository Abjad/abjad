...
def test_intervaltreetools_explode_intervals_uncompactly_01():
...
def test_intervaltreetools_explode_intervals_uncompactly_02():
    '''All resulting trees are non-zero in length.'''
    tree = IntervalTree(_make_test_intervals())
    xtrees = explode_intervals_uncompactly(tree)
    assert all([len(xtree) for xtree in xtrees])

def test_intervaltreetools_explode_intervals_uncompactly_03():
