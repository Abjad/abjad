...
def sequence_to_degree_of_rotational_symmetry(iterable):
   ...
   ::
      ...
      abjad> seqtools.sequence_to_degree_of_rotational_symmetry([1, 1, 2, 1, 1, 1])
      6

   ::

      abjad> seqtools.sequence_to_degree_of_rotational_symmetry([1, 1, 2, 1, 1, 2])
      3

   None when `iterable` is empty. ::

      abjad> seqtools.sequence_to_degree_of_rotational_symmetry([ ]) is None
      True

   .. versionchanged:: 1.1.2
      renamed ``seqtools.get_period( )`` to
      ``seqtools.sequence_to_degree_of_rotational_symmetry( )``.
   '''
