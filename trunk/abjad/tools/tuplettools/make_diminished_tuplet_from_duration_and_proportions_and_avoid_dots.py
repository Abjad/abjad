...
def make_diminished_tuplet_from_duration_and_proportions_and_avoid_dots(duration, proportions):
   ...
   Return tupletted leaves strictly without dots when all `proportions` equal ``1``::
      ...
      abjad> print tuplettools.make_diminished_tuplet_from_duration_and_proportions_and_avoid_dots(Fraction(3, 16), [1, 1, 1, -1, -1])
      {@ 5:3 c'16, c'16, c'16, r16, r16 @}

   Allow tupletted leaves to return with dots when some `proportions` do not equal ``1``::

      abjad> print tuplettools.make_diminished_tuplet_from_duration_and_proportions_and_avoid_dots(Fraction(3, 16), [1, -2, -2, 3, 3])
