...
def test_FixedDurationTuplet_trim_01( ):
   '''1-element index.'''
   t = tuplettools.FixedDurationTuplet((2, 4), Note(0, (1, 8)) * 3)
   for i, leaf in enumerate(t.leaves):
      leaf.pitch = i
   t.trim(0)
   assert len(t) == 2
   assert t[0].pitch.numbered_chromatic_pitch == 1
   assert t[1].pitch.numbered_chromatic_pitch == 2
...
def test_FixedDurationTuplet_trim_02( ):
   ...
   t.trim(1)
   assert len(t) == 2
   assert t[0].pitch.numbered_chromatic_pitch == 0
   assert t[1].pitch.numbered_chromatic_pitch == 2
...
def test_FixedDurationTuplet_trim_03( ):
   ...
   assert len(t) == 2
   assert t[0].pitch.numbered_chromatic_pitch == 0
   assert t[1].pitch.numbered_chromatic_pitch == 1
...
def test_FixedDurationTuplet_trim_05( ):
   ...
   assert t[0].pitch.numbered_chromatic_pitch == 0
   assert t[1].pitch.numbered_chromatic_pitch == 1
   assert t[2].pitch.numbered_chromatic_pitch == 2
