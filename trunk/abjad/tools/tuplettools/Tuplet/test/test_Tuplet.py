...
def test_Tuplet_01():
    r'''Init typical fmtuplet.
    '''

    u = Tuplet(Multiplier(2, 3), Note(0, (1, 8)) * 3)
    ...
    assert inspect(u).get_duration() == Duration(1, 4)


def test_Tuplet_02():
    r'''Init empty fmtuplet.
    '''

    u = Tuplet(Multiplier(2, 3), [])
    assert repr(u) == 'Tuplet(2/3, [])'
    assert str(u) == '{* 2/3 *}'
    assert len(u) == 0
    assert u._preprolated_duration == 0
    assert u.multiplier == Duration(2, 3)
    assert inspect(u).get_duration() == 0
...
def test_Tuplet_03():
    ...
    u = Tuplet(Multiplier(2, 3), [
        Tuplet(Multiplier(4, 5), Note(0, (1, 16)) * 5),
        Note("c'4"),
        Note("c'4")])
    ...
    assert u.multiplier == Duration(2, 3)
    assert inspect(u).get_duration() == Duration(1, 2)
    ...
    assert inspect(u[0]).get_duration() == Duration(1, 6)


def test_Tuplet_04():
    r'''Nest empty fmtuplet.
    '''
    ...
    assert u.multiplier == Duration(2, 3)
    assert inspect(u).get_duration() == Duration(1, 3)
    assert repr(u[0]) == 'Tuplet(4/5, [])'
    assert str(u[0]) == '{* 4/5 *}'
    assert len(u[0]) == 0
    assert u[0]._preprolated_duration == Duration(0)
    assert u[0].multiplier == Duration(4, 5)
    assert inspect(u[0]).get_duration() == Duration(0)
