...
def test_leaftools_make_notes_01( ):
   '''
   leaftools.make_notes can take a single pitch and a single duration.
   '''
   t = leaftools.make_notes(1, (1,4))
...
def test_leaftools_make_notes_03( ):
   '''
   leaftools.make_notes may take a list of pitches and a single duration.
   '''
   t = leaftools.make_notes([1, 2], (1, 4))
   assert len(t) == 2
   assert t[0].pitch.number == 1
   assert t[1].pitch.number == 2

def test_leaftools_make_notes_04( ):
   '''
   leaftools.make_notes may take a single pitch and a list of duration.
   '''
   t = leaftools.make_notes(1, [(1, 8), (1, 4)])
   assert len(t) == 2
   assert t[0].pitch.number == 1
...
def test_leaftools_make_notes_05( ):
   '''
   leaftools.make_notes may take a list of pitches and list of durations.
   '''
   t = leaftools.make_notes([0, 1], [(1, 8), (1, 4)])
   assert len(t) == 2
...
def test_leaftools_make_notes_06( ):
   '''
   Durations can be Rationals.
   '''
   t = leaftools.make_notes(1, Rational(1, 4))
   assert len(t) == 1
   assert t[0].duration.written == Rational(1, 4)

def test_leaftools_make_notes_07( ):
   '''
...
def test_leaftools_make_notes_10( ):
   '''
   make_notes( ) can take parameters for a single prolated note.
   '''
   t = leaftools.make_notes(1, (1, 36))
   assert len(t) == 1
...
def test_leaftools_make_notes_11( ):
   '''
   make_notes( ) can take a single pitch and a list of prolated durations.
   '''
   t = leaftools.make_notes(1, [(1, 12), (1, 6), (1, 8)])
   assert len(t) == 2
   assert isinstance(t[0], FixedMultiplierTuplet)
   assert isinstance(t[1], Note)
   assert len(t[0]) == 2
...
def test_leaftools_make_notes_12( ):
   '''
   make_notes( ) may take a direction attribute.
   '''
   t = leaftools.make_notes(1, [(5, 12), (1, 6), (1, 8)], \
