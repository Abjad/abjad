...
def is_bar_line_crossing_leaf(leaf):
    r'''True when `leaf` crosses bar line:
    ...
    time_signature = more(leaf).get_effective_context_mark(
    ...
    shifted_start = (leaf.get_timespan().start_offset - partial) % time_signature_duration

    if time_signature_duration < shifted_start + leaf.get_duration():
         return True
