...
def test_tiechaintools_duration_change_01( ):
   '''Change length-1 tie chain to length-2 tie chain.'''

   t = Staff(run(1))
   Beam(t[:])
   tiechaintools.duration_change(t[0].tie.chain, Rational(5, 32))
...
def test_tiechaintools_duration_chnage_02( ):
   ...
   Beam(t[:])
   tiechaintools.duration_change(t[0].tie.chain, Rational(4, 32))
