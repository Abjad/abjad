...
def test_init_typical_fmtuplet( ):
   ...
   assert u.duration.resultant == Rational(1, 4)
   assert u.duration.absolute == Rational(1, 4)
...
def test_empty_fmtuplet( ):
   ...
   assert str(u) == '{* 2/3 *}'
   assert len(u) == 0
   assert u.duration.resultant == 0
   assert u.duration.multiplier == Rational(2, 3)
   assert u.duration.absolute == 0
...
def test_nest_typical_fmtuplet( ):
   ...
   assert len(u) == 3
   assert u.duration.resultant == Rational(1, 2)
   assert u.duration.multiplier == Rational(2, 3)
   assert u.duration.absolute == Rational(1, 2)
   assert repr(u[0]) == "FixedMultiplierTuplet(4/5, [c'16, c'16, c'16, c'16, c'16])"
   assert str(u[0]) == "{* 5:4 c'16, c'16, c'16, c'16, c'16 *}"
   assert len(u[0]) == 5
   assert u[0].duration.resultant == Rational(1, 4)
   assert u[0].duration.multiplier == Rational(4, 5)
   assert u[0].duration.absolute == Rational(1, 6)
...
def test_nest_typical_fmtuplet( ):
   ...
   assert len(u) == 3
   assert u.duration.resultant == Rational(1, 3)
   assert u.duration.multiplier == Rational(2, 3)
   assert u.duration.absolute == Rational(1, 3)
   assert repr(u[0]) == 'FixedMultiplierTuplet(4/5, [ ])'
   assert str(u[0]) == '{* 4/5 *}'
   assert len(u[0]) == 0
   assert u[0].duration.resultant == Rational(0)
   assert u[0].duration.multiplier == Rational(4, 5)
   assert u[0].duration.absolute == Rational(0)
