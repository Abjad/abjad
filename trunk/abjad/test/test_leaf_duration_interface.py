...
def test_durations_gt_one_01( ):
   ...
   #t.duration = 3
   t.duration.written = 3
   assert t.format == "cs'\\breve."
   #t.duration = 4
   t.duration.written = 4
   assert t.format == "cs'\\longa"
   #t.duration = 6
   t.duration.written = 6
   assert t.format == "cs'\\longa."
   #t.duration = 7
   t.duration.written = 7
   assert t.format == "cs'\\longa.."
   #t.duration = 8
   t.duration.written = 8
   assert t.format == "cs'\\maxima"
   #t.duration = 12
   t.duration.written = 12
   assert t.format == "cs'\\maxima."
   #t.duration = 14
   t.duration.written = 14
   assert t.format == "cs'\\maxima.."
   #t.duration = 15
   t.duration.written = 15
   assert t.format == "cs'\\maxima..."
