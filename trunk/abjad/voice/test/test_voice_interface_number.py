...
def test_voice_interface_number_03( ):
   ...
   Anything else will throw a ValueError exception.
   '''
   t = Voice(run(4))
   t[0].voice.number = 1
   assert t.format == "\\new Voice {\n\t\\voiceOne\n\tc'8\n\tc'8\n\tc'8\n\tc'8\n}"
   ...
   assert t.format == "\\new Voice {\n\tc'8\n\tc'8\n\tc'8\n\tc'8\n}"
   assert raises(ValueError, 't[0].voice.number = 23')
...
def test_voice_interface_number_05( ):
   '''
   Voice number can be set on a Voice container and on one of the leaves
   contained in it.
   '''
   t = Voice(run(4))
   t.voice.number = 1
