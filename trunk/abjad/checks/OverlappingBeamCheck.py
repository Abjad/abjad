...
class OverlappingBeamCheck(_Check):
   ...
   def _run(self, expr):
      ...
      for leaf in leaftools.iterate_leaves_forward_in_expr(expr):
         ...
         beams = spannertools.get_all_spanners_attached_to_component(leaf, BeamSpanner)
         if 1 < len(beams):
            for beam in beams:
               if beam not in violators:
                  violators.append(beam)
      #total = len([p for p in expr.spanners.contained if isinstance(p, BeamSpanner)])
      total = len(spannertools.get_all_spanners_attached_to_component(expr, BeamSpanner))
