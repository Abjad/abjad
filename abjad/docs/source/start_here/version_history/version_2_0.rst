...
* The ``indicatortools``, ``instrumenttools``, ``intervaltreetools``, ``lilyfiletools``, ``indicatortools``, ``pitcharraytools``, ``quantizationtools``, ``sievetools``, ``tonalanalysistools`` and ``verticalitytools`` packages are completely new.

* The classes implemented in the ``indicatortools`` and ``indicatortools`` packages provide an object-oriented interfaces to clefs, time signatures, key signatures, articulations, tempos and other symbols stuck to the outside of the hierarchical score tree. The classes implemented in ``indicatortools`` and ``indicatortools`` model information outside the score tree much the way that the classes implemented in ``spannertools`` implement object-oriented interfaces to beams, brackets, hairpins, glissandi and other line-like symbols.
