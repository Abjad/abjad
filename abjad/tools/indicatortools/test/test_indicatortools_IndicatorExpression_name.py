...
def test_indicatortools_IndicatorExpression_name_01():
    ...
    indicator_expression = inspect(component).get_indicators(unwrap=False)[0]
...
def test_indicatortools_IndicatorExpression_name_03():
    ...
    indicator_expression_a = inspect(leaf_a).get_indicators(unwrap=False)[0]
    assert indicator_expression_a.name is None
    leaf_b = Note("g'4")
    attach(indicator_expression_a, leaf_b)
    indicator_expression_b = inspect(leaf_b).get_indicators(unwrap=False)[0]
