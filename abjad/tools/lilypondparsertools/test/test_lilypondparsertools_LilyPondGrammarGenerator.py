...
class Test(unittest.TestCase):
    ...
    parser_tab_hh_path = os.path.join(
        os.path.dirname(__file__),
        'lilypond-2.19.24-parser.tab.hh',
        )

    parser_output_path = os.path.join(
        os.path.dirname(__file__),
        'lilypond-2.19.24-parser.output',
        )
    ...
    expected_contents = stringtools.normalize(r"""
    # -*- encoding: utf-8 -*-
    ...
    def test_01(self):
        grammar_generator = lilypondparsertools.LilyPondGrammarGenerator()
        grammar_generator(
            output_path=self.output_path,
            parser_output_path=self.parser_output_path,
            parser_tab_hh_path=self.parser_tab_hh_path,
            )
        assert os.path.exists(self.output_path)
        with open(self.output_path, 'r') as file_pointer:
            actual_contents = file_pointer.read()
        assert self.expected_contents == actual_contents