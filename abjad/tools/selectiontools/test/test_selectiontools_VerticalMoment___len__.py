...
def test_selectiontools_VerticalMoment___len___01():
    ...
    vertical_moment = inspect_(piano_staff).get_vertical_moment_at(Offset(1, 8))
    "VerticalMoment(PianoStaff<<2>>, Staff{2}, a'4, Staff{4}, e'8)"
    assert len(vertical_moment) == 5

    vertical_moment = inspect_(piano_staff[0]).get_vertical_moment_at(Offset(1, 8))
    "VerticalMoment(Staff{2}, a'4)"
    assert len(vertical_moment) == 2

    vertical_moment = inspect_(piano_staff[1]).get_vertical_moment_at(Offset(1, 8))
