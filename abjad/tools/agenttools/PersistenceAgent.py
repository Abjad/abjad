...
class PersistenceAgent(abctools.AbjadObject):
    ...
    def as_ly(self, ly_file_name=None):
        r'''Persists client as LilyPond file.

        If `ly_file_name` is none, a file path will be autogenerated.
        ...
        if ly_file_name is None:
            ...
            ly_file_path = os.path.expanduser(ly_file_name)
    ...
    def as_midi(self, midi_file_name=None):
        ...
        if midi_file_name is not None:
            midi_file_path = os.path.expanduser(midi_file_name)
            ly_file_path = '{}.ly'.format(os.path.splitext(midi_file_path)[0])
        else:
            ly_file_path = None
        ly_file_path, abjad_formatting_time = type(self)(illustration).as_ly(
            ly_file_path)
        timer = systemtools.Timer()
        with timer:
    ...
    def as_module(self, module_file_name, object_name):
        ...
        result = '\n'.join(result)
        module_file_path = os.path.expanduser(module_file_name)
        systemtools.IOManager.ensure_directory_existence(
            os.path.dirname(module_file_path))
        with open(module_file_path, 'w') as f:
            f.write(result)

    def as_pdf(self, pdf_file_name=None):
        r'''Persists client as PDF.

        If `pdf_file_name` is none, a file path will be autogenerated.
        ...
        if pdf_file_name is not None:
            pdf_file_path = os.path.expanduser(pdf_file_name)
            ly_file_path = '{}.ly'.format(os.path.splitext(pdf_file_path)[0])
        else:
            ly_file_path = None
        # format and write the lilypond file
        ly_file_path, abjad_formatting_time = self.as_ly(ly_file_path)
        pdf_file_path = '{}.pdf'.format(os.path.splitext(ly_file_path)[0])
        # render the pdf
        timer = systemtools.Timer()
        with timer:
            systemtools.IOManager.run_lilypond(ly_file_path)
