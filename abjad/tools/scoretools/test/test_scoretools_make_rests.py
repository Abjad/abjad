...
def test_scoretools_make_rests_01():
    ...
    assert len(inspect(rest[0]).get_logical_tie()) == 1
...
def test_scoretools_make_rests_02():
    ...
    assert all(len(inspect(x).get_logical_tie()) == 1 for x in rest)
...
def test_scoretools_make_rests_03():
    ...
    assert all(len(inspect(x).get_logical_tie()) == 2 for x in true)
...
def test_scoretools_make_rests_04():
    ...
    assert all(len(inspect(x).get_logical_tie()) == 1 for x in t)
