...
def test_pitchtools_PitchArray_from_score_03():
    ...
    assert pitch_array[0].pitches == tuple(iterate(score[0]).by_pitch())
    assert pitch_array[1].pitches == tuple(iterate(score[1]).by_pitch())
    assert pitch_array[2].pitches == tuple(iterate(score[2]).by_pitch())
