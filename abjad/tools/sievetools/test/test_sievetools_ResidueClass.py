...
def test_sievetools_ResidueClass_04():
    ...
    assert residue_class.offset == 0
    assert residue_class.get_congruent_bases(4) == [0, 2, 4]
...
def test_sievetools_ResidueClass_05():
    ...
    assert residue_class.get_congruent_bases(5) == [1, 3, 5]
...
def test_sievetools_ResidueClass_06():
    ...
    assert residue_class.offset == 0
    assert residue_class.get_congruent_bases(6) == [0, 3, 6]
...
def test_sievetools_ResidueClass_07():
    ...
    assert residue_class.period == 3
    assert residue_class.offset == 1
    assert residue_class.get_congruent_bases(7) == [1, 4, 7]
...
def test_sievetools_ResidueClass_08():
    ...
    assert isinstance(residue_class.get_congruent_bases(12), list)
    assert isinstance(residue_class.get_boolean_train(12), list)

    assert residue_class.get_congruent_bases(99) == list(range(100))
